...
export function setCookie(key: string, value: string, day: number, path?: string, domin?: string) {
  ...
  const cookiePath = path || currentIndex;
  // 2.处理默认保存的domin(域名)
  const cookieDomin = domin || document.domain;

  // 3.处理输入的时间
  if (!day) {
    document.cookie = `${key}=${value};path=${cookiePath};domin=${cookieDomin}`;
  } else {
    const date = new Date();
    date.setDate(date.getDate() + day);
    document.cookie = `${key}=${value};expires=${date.toUTCString()};path=${cookiePath};domin=${cookieDomin}`;
  }
}
